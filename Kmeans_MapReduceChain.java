...
public class MapReduceChain extends Configured implements Tool {
    public int run(String[] args) throws Exception {
        ...
        if (JobClient.runJob(job1Conf).isSuccessful()) {
            String outputFirstJob = "/Part1_Output1_Means/part-00000";
            ...
            String newCentersDir = "centersMeans_Chain";

            try {
                File centersMeansDir = new File(newCentersDir);
                centersMeansDir.mkdir();

                File originalCenters = new File(previousCentersFile);
                Files.move(originalCenters.toPath(), new File(centersMeansDir, previousCentersFile).toPath(), StandardCopyOption.REPLACE_EXISTING);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
	...
	if (JobClient.runJob(job2Conf).isSuccessful()) {
            String outputFirstJob = "/Part1_Output2_Means/part-00000";
            String localCentersFile = "centers2.txt";
            String[] copyToLocalArgs = {"-copyToLocal", outputFirstJob, localCentersFile};
            ToolRunner.run(new Configuration(), new org.apache.hadoop.fs.FsShell(), copyToLocalArgs);

            String previousCentersFile = "centers1.txt";
	    String existingCentersDir = "centersMeans_Chain";
	...
	if (JobClient.runJob(job3Conf).isSuccessful()) {
            String outputFirstJob = "/Part1_Output3_Means/part-00000";
	...
	if (JobClient.runJob(job4Conf).isSuccessful()) {
            String outputFirstJob = "/Part1_Output4_Means/part-00000";
